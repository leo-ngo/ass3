function loadDrugs(){
        $('#drug_list').html('')
        //$('#drug_list').append(showSearch())

        fetch(baseURL + '/drugs', {headers: {'Authorization': 'Basic ' + code}})
        .then((res) => res.json())
        .then((json) => {
...
function showSearch(){
    var str = '<tr>\
              ...
              <th scope="col">\
                  <input type="text" class="form-control input-sm" id="myInput2" onkeyup="searchUsage()" placeholder="Usage">\
              </th>\
              <th></th>\
              <th></th>\
              </tr>'
